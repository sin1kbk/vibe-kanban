...
description: "Run Vibe Kanban Cloud locally for development and testing"
---
...
# Required - Password for ElectricSQL database role (electric_sync user)
ELECTRIC_ROLE_PASSWORD=your_secure_password_here

# GitHub OAuth (required if using GitHub login)
...
# Email invitations (required but can be empty to disable)
LOOPS_EMAIL_API_KEY=
```

<Info>
The `ELECTRIC_ROLE_PASSWORD` is used to create a database role called `electric_sync` that ElectricSQL uses for real-time replication. The remote server creates this role automatically on startup.
</Info>
...
<Info>
The first build takes several minutes as it compiles the Rust backend. Subsequent starts are much faster.
</Info>
...
<AccordionGroup>
<Accordion title="Build fails with SSH errors">
The Dockerfile uses SSH for accessing private dependencies during build. If you see SSH errors:
...
# Then run docker compose with SSH forwarding
DOCKER_BUILDKIT=1 docker compose --env-file ../../.env.remote up --build
```
</Accordion>
